...
//	private static final String GET_ALL_SALES = "SELECT * FROM sales_record";  //取得全部資料
	...
	private static final String CLEAN_PAY_ONCOUNTER = "UPDATE member_overview SET onCounter = 0";
	...
	public boolean cleanPayAll() {
		...
		try {
			...
			stmt.execute();
			stmt = conn.prepareStatement(CLEAN_PAY_ONCOUNTER);
			stmt.execute();
			stmt.close();
			delStatus = true;
		} catch (SQLException e) {
...
//	        	System.out.println(memberNumber);
				...
				cstmt.execute();
				PreparedStatement stmt = conn.prepareStatement(CLEAN_INVENTORY);
				stmt.execute();
				...
				System.out.println("insert Stored Procedure successful!");
				cstmt.close();
				stmt = conn.prepareStatement(CLEAN_PAY_ALL);
				stmt.execute();
